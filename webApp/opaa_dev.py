...
global model, graph, api
# initialize these variables
model, graph, api,twitter = init()
#f_model = init()
@app.route('/')
def hello(st=''):
    ...
    for i in results:
        for trend in i['trends']:
            ...
            temp = temp + 1
    num = [1,2,3,4,5]
    topics = zip(trends,num)
    return render_template('home.html',title='Index',topics=topics)
...
def analysis():
    ...
    #Use twitter developer credentials below and uncomment following commented code
    """
    ...
    with graph.as_default():
        ...
        df2 = pd.read_csv('emoji_sentiment.csv')        
        x = df2['emoji'].values
        y = df2['sentiment_score'].values
        dict = {}
        ...
        for q in range(len(em)):
            if(em[q]!='nan'):
                ...
                sc_list = em[q].split(",")
                #print(sc_list)
                #print(type("Type:",dict.keys))
                for emj in sc_list:
                    if emj in dict.keys():
                        em_sent_score = em_sent_score + dict[emj]
                if(em_sent_score<0):
                    em_sent_score = 0 - em_sent_score
                    senModelList[q][0] = senModelList[q][0] + em_sent_score
                    new_neg = (senModelList[q][0])/(senModelList[q][0]+senModelList[q][1])
                    new_pos = (senModelList[q][0])/(senModelList[q][0]+senModelList[q][1])
                    senModelList[q] = [new_neg,new_pos]
                elif(em_sent_score>0):
                    senModelList[q][0] = senModelList[q][0] + em_sent_score
                    new_neg = (senModelList[q][0])/(senModelList[q][0]+senModelList[q][1])
                    new_pos = (senModelList[q][0])/(senModelList[q][0]+senModelList[q][1])
                    senModelList[q] = [new_neg,new_pos]
        ...
        #tw_sent = dataset['sentiment'].values.tolist()
